...
export default function ProductCard({ product, onAddToCart }) {
  ...
  return (
    <div className="group flex flex-col bg-white rounded-2xl border border-stone-200/60 overflow-hidden hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all duration-300">
      <Link to={`/product/${product.id}`} className="block aspect-[4/3] bg-stone-50 overflow-hidden relative">
        <img
          src={image}
          alt={product.name}
      ...
      <div className="p-5 flex flex-col flex-1">
        <Link to={`/product/${product.id}`} className="hover:underline decoration-stone-400 underline-offset-4">
          <h3 className="font-medium text-stone-900 text-lg leading-snug truncate">{product.name}</h3>
        </Link>
